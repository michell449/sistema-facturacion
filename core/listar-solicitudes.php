...
function ls_html_escape(string $v): string { return htmlspecialchars($v, ENT_QUOTES, 'UTF-8'); }

try {
    $db = (new Database())->getConnection();
    $cols = [];
    try { $cols = $db->query('SHOW COLUMNS FROM cf_solicitudes')->fetchAll(PDO::FETCH_COLUMN); } catch (Throwable $e) {}
    ...
    $where = [];$params=[];
    if ($fRfc !== '' && $rfcCol!=='NULL') { $where[] = "$rfcCol LIKE ?"; $params[] = "%$fRfc%"; }
    if ($fTipo !== '' && in_array($fTipo,['emitidas','recibidas','emitidos'])) {
        if ($fTipo==='emitidas') { $where[]="(tipo='emitidas' OR tipo='emitidos')"; } else { $where[]='tipo=?'; $params[]=$fTipo; }
    }
    if ($fEstado!=='' && in_array($fEstado,['pendiente','aceptada','terminada','rechazada','error','vencida'])) { $where[]='estado=?'; $params[]=$fEstado; }
    if ($fTexto!=='') { $where[]="(solicitud_id_sat LIKE ? OR $rfcCol LIKE ? OR tipo LIKE ?)"; $params[]="%$fTexto%"; $params[]="%$fTexto%"; $params[]="%$fTexto%"; }
    ...
    foreach ($rows as $r) {
        $paquetesDesc = 0; $total = (int)($r['total_paquetes'] ?? 0);
